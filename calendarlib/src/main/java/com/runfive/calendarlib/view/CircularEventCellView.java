package com.runfive.calendarlib.view;
...
import com.runfive.calendarlib.R;
...
public class CircularEventCellView extends BaseCellView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if((stateSet==null || stateSet.isEmpty()
                ||(stateSet.size() ==1 && stateSet.contains(BaseCellView.STATE_REGULAR))) && paintList!=null) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if((stateSet==null || stateSet.isEmpty() || (stateSet.size() ==1
                && stateSet.contains(BaseCellView.STATE_REGULAR))) && paintList!=null) {
            int num = paintList.size();
            for (int i=0;i<num;i++) {
